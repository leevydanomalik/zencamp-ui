...
class OrderProduk extends Component  {
  constructor(props) {
      super(props)
      const judul = ''
      this.state = {
  ...
  async getData() {
      ...
      if (response && response.status === 'S') {
          ...
          let dataTable = dataMhs.map((value, index) => {
              const { msoID, msoName, msoPeriod, msoDocDate, msoVendor, msoDriver,msoFleet, msoFrom, msoDestination } = value
              return [index += 1, msoID, msoName, msoPeriod, msoDocDate, msoVendor, msoDriver,msoFleet, msoFrom, msoDestination  ]
          })
  ...
  async saveEdit(data) {
      let payload = {
          ...data,
          msoDocDate: M(data.msoDocDate, 'YYYY-MM-DD').format('DD-MM-YYYY'),
          msoMaterials: [{...data.msoMaterials,
            msStatus: "",
            msType: ""
